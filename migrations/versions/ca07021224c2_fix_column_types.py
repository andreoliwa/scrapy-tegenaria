...
def downgrade():
    ...
    op.alter_column(
        "apartment", "size", existing_type=sa.Numeric(precision=4, scale=1), type_=sa.VARCHAR(), existing_nullable=True
    )
    op.alter_column(
        "apartment", "rooms", existing_type=sa.Numeric(precision=2, scale=1), type_=sa.VARCHAR(), existing_nullable=True
    )
    op.alter_column(
        "apartment",
        "cold_rent",
