...
class AkeliusSpider(CrawlSpider, SpiderMixin):
    ...
    start_urls = ["https://www.akelius.de/en/search/apartments/osten/berlin/list"]

    rules = (Rule(LinkExtractor(allow=r"berlin/[0-9\.]+", process_value=url_query_cleaner), callback="parse_item"),)
    ...
    def parse_item(self, response):
        ...
        item.add_xpath(
            "warm_rent_price", '//h2/following-sibling::p[starts-with(normalize-space(.), "Total rent")]/text()'
        )
        item.add_xpath("size", "//h2//following-sibling::p[2]/text()")
        item.add_xpath(
            "location", '//h3[starts-with(normalize-space(.), "Location")]/following-sibling::div//span/text()'
        )
        ...
        item.add_xpath(
            "description", '//h3[starts-with(normalize-space(.), "Building")]/following-sibling::div//span/text()'
        )
