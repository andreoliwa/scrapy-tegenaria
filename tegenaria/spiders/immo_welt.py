...
class ImmoWeltSpider(CrawlSpider, SpiderMixin):
    ...
    def parse_item(self, response):
        ...
        item.add_xpath(
            "cold_rent_price",
            '//div[contains(@class, "hardfacts")]/div[contains(@class, "hardfact")][1]' "/strong/text()",
        )

        item.add_xpath(
            "description",
            '//div[contains(@class, "section_label")][starts-with('
            'normalize-space(.), "Objekt")]/following-sibling::div/child::p/text()',
        )

        for field, cell_text in {
            "warm_rent_price": "Warmmiete",
            "additional_price": "Nebenkosten",
            "heating_price": "Heizkosten",
        }.items():
            item.add_xpath(
                field,
                '//div[contains(@class, "datatable")]/div[contains(@class, "datarow")]/div[contains'
