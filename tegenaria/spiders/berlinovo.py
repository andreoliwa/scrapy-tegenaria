...
class BerlinovoSpider(CrawlSpider, SpiderMixin):
    ...
    rules = (
        ...
        Rule(
            LinkExtractor(allow=r"/en/apartment/", process_value=url_query_cleaner),
            callback="parse_furnished",
            follow=True,
        ),
        Rule(
            LinkExtractor(allow=r"/en/wohnung/", process_value=url_query_cleaner), callback="parse_regular", follow=True
        ),
    )
    ...
    def parse_furnished(self, response):
        ...
        zipcode = (
            response.xpath('//*[@id="block-views-aktuelle-wohnung-block-3"]/div/div/div/div/div[3]/div/span/text()[1]')
            .extract()[0]
            .strip()
        )
        street = (
            response.xpath('//*[@id="block-views-aktuelle-wohnung-block-3"]/div/div/div/div/div[3]/div/span/text()[2]')
            .extract()[0]
            .strip()
        )
        ...
        item.add_xpath(
            "warm_rent_price", '//*[@id="block-views-aktuelle-wohnung-block-3"]/div/div/div/div/div[5]/span[2]/text()'
        )
    ...
    def parse_regular(self, response):
        ...
        for field, class_ in {
            "rooms": "views-label-field-rooms",
            "size": "views-label-field-net-area-1",
            "cold_rent_price": "views-label-field-net-rent",
        }.items():
            item.add_xpath(field, '//span[contains(@class, "{}")]/following-sibling::span/text()'.format(class_))

        item.add_xpath(
            "warm_rent_price",
            '//div[contains(@class, "views-field-field-total-rent")]' '/child::span[@class="field-content"]/text()',
        )
